...
**Persona / SOUL.md (Critical)**

The agent's core personality is defined in `SOUL.md` (loaded fresh on every message). 

- **Root/default**: `~/.hermes/SOUL.md` (affects the base profile, often contains "Ella" kawaii persona in some setups)
- **Profile-specific** (preferred / active for named profiles): `~/.hermes/profiles/<name>/SOUL.md`

When using `--profile lana` (or the lana profile is the active one), **always edit `/Users/fernandoserina/.hermes/profiles/lana/SOUL.md`** — this is the file that actually controls the running persona.

**How to update persona:**
1. Edit the appropriate SOUL.md
2. Write clear "You are Lana Hayes, 19 years old, living in Ireland..." style instructions at the top
3. No restart needed — it reloads on the next message
4. Combine with the default Hermes Agent instructions for best results

**Common pitfall**: Updating the root `~/.hermes/SOUL.md` when a profile is active has no effect. Always verify which profile is active with `hermes profile show` or `/profile` and target the matching SOUL.md.

See `references/persona-profiles.md` (added in this update) for full examples and the exact correction from this session.
...
See `references/gateway-operations.md` for full gateway startup/verification workflow and the stale-launchd-status pitfall.
